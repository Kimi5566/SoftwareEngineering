...
class opencv_engine(object):
    ...
    def draw_rectangle_by_xywh(img, xywh=(0, 0, 0, 0), color=(0, 0, 255)):  # red
        ...
        right_down = opencv_engine.point_float_to_int(
    ...
    @staticmethod
    def writevideofile(vc, file_name):
        fourcc = cv2.VideoWriter_fourcc(*'MJPG')
        out = cv2.VideoWriter(file_name+'.avi', fourcc,
                              30.0, (1920, 1080), True)
        number = 0
        while number < int(vc.get(cv2.CAP_PROP_FRAME_COUNT)):
            number = number+1
            ret, frame = vc.read()
            if not ret:
                print("end")
